...
public class CategoryController {
	...
	public ResponseEntity<?> getAllCategories() {
		try {
			...
			if (!categories.isEmpty()) {
				...
				return ResponseEntity.status(HttpStatus.OK).body("Não há categoria cadastrada"); // Retorna 204 No
																									// Content se a
																									// lista estiver
																									// vazia
			}
	...
	@Operation(summary = "Listar a categoria por ID", description = "Retorna uma categoria específica.", tags = {
			"Get" })
	@ApiResponses({
			@ApiResponse(responseCode = "200", content = {
					@Content(schema = @Schema(implementation = Category.class), mediaType = "application/json") }),
	...
	public ResponseEntity<?> getCategoryById(@Valid @PathVariable Long id) {
		try {
			...
			if (category.isPresent()) {
				...
				return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Serviço não encontrado com o ID: " + id); // Retorna
																													// 404
																													// Not
																													// Found
																													// se
	...
	public ResponseEntity<?> createCategory(@Valid @RequestBody Category newCategory, BindingResult bindingResult) {
		...
		try {
			...
			if (categoryRepository.existsByName(newCategory.getName())) {
				return ResponseEntity.badRequest().body("Já existe uma categoria com o mesmo nome"); // Retorna 400 Bad
																										// Request se a
																										// categoria já
																										// existir
	...
	public ResponseEntity<?> updateCategory(@PathVariable Long id, @RequestBody Category updatedCategory) {
		try {
			...
			if (optionalCategory.isPresent()) {
				...
				return ResponseEntity.ok("Categoria atualizada com sucesso"); // Retorna a categoria atualizada com
																				// sucesso
			} else {
				return ResponseEntity.status(HttpStatus.NOT_FOUND)
						.body("Não foi encontrada uma categoria com o ID: " + id); // Retorna 404 Not Found se a
	...
	public ResponseEntity<?> deleteCategory(@PathVariable Long id) {
		try {
			...
			if (optionalCategory.isPresent()) {
				...
				return ResponseEntity.ok().body("Categoria deletada com sucesso"); // Retorna 200 Ok após a exclusão da
																					// categoria
			} else {
				return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Categoria não encontrada com o ID: " + id); // Retorna
																														// 404
																														// Not
																														// Found
																														// se
